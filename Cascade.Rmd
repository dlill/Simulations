...
Function to Calculate the dose-response plot
```{r}
...
plot((x * p_log_fitting)(seq(0,1000,length.out = 50), pars, deriv = F))
which_par <- "loggainlf"
dr <- r_opt_dose_response(which_par = which_par, seq(-2,2,by = 0.1), pars = pars, alpha_scan_range = c(-6, seq(-3,4, by = 0.5)))

saveRDS(dr, file = paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."), ".rds"))
saveRDS(pars, file = paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."), "_pars.rds"))
```
...
which_par <- "loggainlf"
dr <- readRDS(paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."), ".rds"))
```
...
dr %>% 
  ...
  filter(alpha < -5.8 |  near(alpha,0) | near(alpha,1)  ) %>%
  # filter(dose < 4) %>%
  ...
  # plotly::ggplotly()
ggsave(paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."),"facet~alpha", ".pdf"), device = "pdf")
ggsave(paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."),"facet~alpha", ".png"), device = "png")
```
...
dr %>%   
  ...
  # plotly::ggplotly()
ggsave(paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."),"facet~dose", ".pdf"), device = "pdf")
ggsave(paste0("Output/drplots/", which_par, paste(steady_states, collapse = "."),"facet~dose", ".png"), device = "png")
```
