...
p_nosyndeg <- P(myp, condition = "one")
...
# run benchmark to compare runtimes
```{r, eval=FALSE}

optim1 <- rbenchmark::benchmark({mstrust(obj_alpha, center =  pars_opt_0+0.5, studyname = "Fits", cores = 1, fits = 1, sd = 0.3, mypars = pars)}, replications = 10)

optim2 <- rbenchmark::benchmark({
  optim(pars_opt_0+runif(3,0,10),
        obj_alpha2, mypars = pars)}, replications = 10)

optim3 <- rbenchmark::benchmark({
  optim(pars_opt_0+runif(3,0,10),
        obj_alpha2, method = "BFGS", mypars = pars)}, replications = 10)
...
optim5 <- rbenchmark::benchmark({
  optim(pars_opt_0+runif(3,0,10),
        obj_alpha2, method = "L-BFGS-B", mypars = pars)}, replications = 10)
...
fits1 <- mstrust(obj_alpha, center =  pars_opt_0, samplefun = "runif", studyname = "Fits", cores = 4, fits = 8, max = 10, mypars = pars)
fits2 <- mclapply(1:8, function(i) optim(pars_opt_0+runif(3,0,10),obj_alpha2, mypars = pars),mc.cores = 4)
fits3 <- mclapply(1:8, function(i) optim(pars_opt_0+runif(3,0,10),obj_alpha2, mypars = pars, method = "BFGS"),mc.cores = 4)
fits5 <- mclapply(1:8, function(i) optim(pars_opt_0+runif(3,0,10),obj_alpha2, mypars = pars, method = "L-BFGS-B"),mc.cores = 4)
...
```{r}
fits1 %>% as.parframe()
fits2 %>% lapply("[[", 2) %>% do.call(c,.) %>% sort()
fits3 %>% lapply("[[", 3) %>% do.call(c,.) %>% sort()
fits5 %>% lapply("[[", 3) %>% do.call(c,.) %>% sort()
```
