---
title: "Cascade-feedback-analysis-pertub-v1k4k7"
author: "Daniel Lill"
output:
  pdf_document:
    keep_tex: yes
...
fit_name <- "Cascade-feedback-analysis-pertub-v1k4k7"
```
...
```{r Setting 1}
obs_fun_list[[1]]   <-  g0
pars_perturbed_list[[1]] <- c("v1" = 1.1, "k4" = 1.1, "k7" = 1.1)
...
ic_list_list[[1]] <- ic_list <- list(gainlf = c(0,1,5,50),
                                     gainuf = c(0,1,5,50))
...
```{r Setting 2}
obs_fun_list[[2]]   <-  g0
pars_perturbed_list[[2]] <- c("v1" = 1.1, "k4" = 1.1, "k7" = 1.1)
                                       
data <- data_fun(50,5,50,5,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
...
ic_list_list[[2]] <- ic_list <- list(gainlf = c(0,1,5,50),
                                     gainuf = c(0,1,5,50))
...
```{r Setting 3}
obs_fun_list[[3]]   <-  g0
pars_perturbed_list[[3]] <- c("v1" = 1.1, "k4" = 1.1, "k7" = 1.1)
                                       
data <- data_fun(50,15,50,15,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
...
ic_list_list[[3]] <- ic_list <- list(gainlf = c(0,1,5,50),
                                     gainuf = c(0,1,5,50))
...
```{r Setting 4}
obs_fun_list[[4]]   <-  g0
pars_perturbed_list[[4]] <- c("v1" = 1.1, "k4" = 1.1, "k7" = 1.1)
...
ic_list_list[[4]] <- ic_list <- list(gainlf = c(0,1,5,50),
                                     gainuf = c(0,1,5,50))
