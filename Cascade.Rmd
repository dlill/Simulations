...
# logtrafo %>% as.numeric()
# myfit[[3]]$gradient[is.na(as.numeric(logtrafo))]
# myfit[[1]]$hessian[is.na(as.numeric(logtrafo)),is.na(as.numeric(logtrafo))] %>% svd %>% extract2("d")
# myfit[[1]]$hessian %>% svd %>% extract2("d")
# profiles <- profile(obj, pars, names(pars)[is.na(as.numeric(logtrafo))], cores = 2 )
```
...
fits1 <- runtime(mstrust(obj_alpha, center =  pars_opt_0 + 36, studyname = "Fits", cores = 3, fits = 12, mypars = pars))
fits2 <- runtime(mclapply(1:12, function(i) optim(pars_opt_0+rnorm(2,36),obj_alpha_no_derivs, mypars = pars),mc.cores = 3))
# fits3 <- runtime(mclapply(1:12, function(i) optim(pars_opt_0+rnorm(2,36),obj_alpha_no_derivs, mypars = pars, method = "BFGS"),mc.cores = 3))
# fits5 <- runtime(mclapply(1:12, function(i) optim(pars_opt_0+rnorm(2,36),obj_alpha_no_derivs, mypars = pars, method = "L-BFGS-B"),mc.cores = 3))
...
```{r}
dr <- r_opt_dose_response("loggainuf", seq(-2,2,by = 0.1), pars = pars, alpha_scan_range = seq(-6,5, by = 0.2))
```

```{r}
dr %>% 
  filter(alpha %in% c(-5,-1,1,3)) %>%
...
```{r}
dr %>% 
  filter(dose %in% seq(-2,2,0.4)[4:8]) %>%
  gather(key = "matrix", value  = "value", r_alpha, r_kept, r_opt) %>% 
