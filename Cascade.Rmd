...
logmypars_opt <- paste0("log", mypars_opt)
logpars_opt_default <- rep(-9, length(logmypars_opt)) %>% set_names(logmypars_opt)

p_log_fitting_defaults <- as.parvec(c(c(loggainlf = 0,
                                 loggainuf = 0,
                                 logk9 = log(50),
                                 logk10 = log(50),
                                 logx1 = log(100),
...
SI_pars <- c(100,0,0,100,0,0,100,0,0,
          ...
          550,13.3,100,100,1,1,0,0,0,0,0,0,00,0,0,0,0,0,rep(0,9)) # large complex concentrations

SI_pars2 <- c(100,0,0,100,0,0,100,0,0,
          66,0.45,620,2.4,0.001,0.02,0.4,
          500,22,0.0002,0.035,0.092,
          900,10,100,100,1,1,0,0,0,0,0,0,00,0,0,0,0,0,rep(0,9)) # lower
...
# Dose response plots
```{r, eval=FALSE}
pars <- log(SI_pars) %>% set_names(names(pars_0))# large complex concentrations
pars[pars==(-Inf)] <- -36
