---
title: "Cascade - Perturbation comparison"
author: "Daniel Lill"
output:
  pdf_document:
    keep_tex: yes
...
pars_perturbed_list <- vector("list", 12)
ic_list_list <- vector("list", 12)
pars_list <- vector("list", 12)
obs_fun_list <- vector("list", 12)
...
```{r Setting 1}
obs_fun_list <- lapply(1:12, function(i) g0)

pars_perturbed_list[[1]] <- pars_perturbed_list[[4]] <-pars_perturbed_list[[7]] <-pars_perturbed_list[[10]] <- c("x1" = 0.8, "y1" = 0.8, "z1" = 0.8)
pars_perturbed_list[[2]]<-pars_perturbed_list[[5]] <-pars_perturbed_list[[8]] <-pars_perturbed_list[[11]]  <- c("v1" = 0.8, "k4" = 0.8, "k7" = 0.8)
pars_perturbed_list[[3]]<-pars_perturbed_list[[6]] <-pars_perturbed_list[[9]] <-pars_perturbed_list[[12]]  <- c("k1" = 0.8, "k31" = 0.8, "k61" = 0.8)
...
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[7]] <-pars_list[[8]] <-pars_list[[9]] <- pars <- (myfit %>% 
                                                               as.parframe() %>% 
                                                               as.parvec() %>% 
                                                               p_nosyndeg() %>% 
                                                               extract2(1))
...
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[10]] <-pars_list[[11]] <-pars_list[[12]] <- pars <- (myfit %>% 
                                                               as.parframe() %>% 
                                                               as.parvec() %>% 
                                                               p_nosyndeg() %>% 
                                                               extract2(1))
...
ic_list_list <- lapply(1:12, function(i) list(gainlf = c(1,0,5),
                                              gainuf = c(1,0,5),
                                              gainyf = c(1,0,5)))
