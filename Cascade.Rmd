...
myfits <- mstrust(obj_alpha, center =  (pars_opt_0-pars_opt_0)[c(1,3)], studyname = "Fits", cores = 3, fits = 3, sd = 3, mypars = pars, perturbation_prediction = perturbation_prediction, r_kept = r_kept_0)
# myfits %>% as.parframe() 
...
dr_list2 <- lapply(seq_along(par_opt_settings), function(i) {
  r_opt_dose_response(which_par = "loggainlf",
                      dosages = c(-9,0,1,log(c(10,30,50,100))),
                      pars = pars,
                      alpha_scan_range = seq(-3,3,by = 0.2),
                      pars_opt = pars_opt_0[par_opt_settings[[i]]])
})
