...
ssdr <- dose_response_fun(which_par, dosages, pars = pars, predfun = (xs*p_log * p_pert))
```
...
r_opt_dose_response <- function(which_par,
  ...
  out <- lapply(dosages, function(dose) {
    ...
    # names(fits) <- alpha_scan_range[1:length(fits)] %>% as.character()
    fits <- fits[!sapply(fits,is.null)]
    # names(best_fits) <- alpha_scan_range[1:length(best_fits)] %>% as.character()
    best_fits <- best_fits[!sapply(best_fits,is.null)]
    
    return(list(r_df, fits, best_fits))
  }) 
  
  r_df <- out %>% lapply("[[", 1) %>% do.call(rbind,.)
  fits <- out %>% lapply("[[", 2)
  best_fits <- out %>% lapply("[[", 3)

  out <- r_df
  attr(out, "fits") <- fits
  attr(out, "best_fits") <- best_fits
...
which_par <- "loggainlf"
dr <- r_opt_dose_response(which_par = "loggainlf", c((-9):(-3),seq(-3,3, by = 0.05)), pars = pars, alpha_scan_range = seq(0,1, by = 1))
...
which_par <- "loggainlf"
dr <- readRDS(paste0("Output/drplots_all_alphas_at_once/", 
                     "r_kept varying ", 
                     which_par, 
                     # paste(steady_states, collapse = "."), 
                     ".rds"))
