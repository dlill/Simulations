...
pars <- log(pars+.Machine$double.eps) %>% set_names(names(pars_0))# large complex concentrations
# p_log(pars)
...
r_0 <-   R_fun(pars_opt = pars_opt[c(1,2)],
               perturbation_prediction = perturbation_prediction,
               obs_fun = g,
               p_fun = (p_log*p_pert),
...
r_1<-   R_fun(pars_opt = pars_opt[c(1,2)]-(log(0+.Machine$double.eps)),
               perturbation_prediction = perturbation_prediction,
               obs_fun = g,
               p_fun = (p_log*p_pert),
               pars = pars) %>% local_response_matrix_eq10()
...
myfits <- mstrust(obj_alpha, center =  (pars_opt_0-pars_opt_0)[c(1,2)], studyname = "Fits", cores = 3, fits = 9, sd = 3, mypars = pars, perturbation_prediction = perturbation_prediction)
myfits %>% as.parframe() 
