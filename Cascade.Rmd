...
newsteadystate <- function(steady_states, totals = c(100,100,100)) {
  newss <- c(
    ...
    steady_states[1],steady_states[2],
    totals[2]-steady_states[1]-steady_states[3]-steady_states[4],
    steady_states[3],steady_states[4],
...
find_pars <- function(steady_states, totals = rep(100, 3)) {
  ...
  set.seed(1)
  myfit <- mstrust(obj, pars_0, cores = 3, fits = 6, studyname = "FindPars", resultPath = ".Findpars", sd = 3)
...
pars <- find_pars(steady_states = c(30,2,30,2,30,0))
...
r_opt_dose_response <- function(which_par,
  ...
  out <- lapply(dosages, function(dose) {
    ...
    r_0 <- r_alpha_fun(pars_opt = pars_opt_0-pars_opt_0 + min(alpha_scan_range),
                       perturbation_prediction = perturbation_prediction,
                       obs_fun = obs_fun,
                       p_fun = p_fun,
                       pars = pars)
    ...
    r_kept_df <- sapply(alpha_scan_range, function(a) {
      ...
      group_by(r_element) %>% 
      summarise(r_kept = any(r_kept > 0.5)) %>% 
      mutate(r_kept = add(r_kept, seq(0,0.05,length.out = 9)) )
    ...
    myfits <- mstrust(obj,
                      center =  pars_opt_0-pars_opt_0 - 2, # center around log(pars_opt_0) = -2
    ...
    r_opt_df <- r_alpha_fun(pars_opt = mybest_fit,
                         perturbation_prediction = perturbation_prediction,
                         obs_fun = obs_fun,
                         p_fun = p_fun,
                         pars = pars) %>% 
    ...
    r_df <- data.frame(r_df, dose = dose)
...
plot((x * p_log_fitting)(seq(0,1000,length.out = 50), pars, deriv = F))
which_par <- "loggainlf"
dr <- r_opt_dose_response(which_par = which_par, seq(-2,2,by = 0.1), pars = pars, alpha_scan_range = seq(-15,15, by = 0.1))

saveRDS(dr, file = paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."), ".rds"))
saveRDS(pars, file = paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."), "_pars.rds"))
```
...
which_par <- "loggainlf"
dr <- readRDS(paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."), ".rds"))
```
...
dr %>% 
  ...
  # plotly::ggplotly()
ggsave(paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."),"facet~alpha", ".pdf"), device = "pdf")
ggsave(paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."),"facet~alpha", ".png"), device = "png")
```
...
dr %>%   
  ...
  # plotly::ggplotly()
ggsave(paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."),"facet~dose", ".pdf"), device = "pdf")
ggsave(paste0("Output/drplots_all_alphas_at_once/", which_par, paste(steady_states, collapse = "."),"facet~dose", ".png"), device = "png")
```
