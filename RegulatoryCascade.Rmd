---
title: "RegulatoryCascade-with-noise"
author: "Daniel Lill"
output:
  pdf_document:
    keep_tex: yes
...
fit_name <- "RegulatoryCascade-with-noise"
```
...
pars_opt_0 <- setdiff(attr(g, "parameters"),names(c(pars_raw,ic_raw)))
...
pars_perturbed_0 <- pars_perturbed <- c(x1 = 1.1, y1 = 1.1, z1 = 1.1)
```
...
best_fit <- best_fit[best_fit$a_2 > 0, ] %>% as.parvec()
```
...
```{r Feedback strengths, eval=FALSE}
# mymodel
# pars["k10"]
...
# 3. Set the gains to 1 and the kinetic constants k9, k10, k11 to 100
...
# Data generation
newsteadystate <- function(x2,y2,cyz,z2, xT=100,yT=100,zT=100) {
  newss <- c(xT-x2,
    x2,
    yT-y2-cyz,
    y2,cyz,
...
data_fun <- function(x2,y2,cyz,z2, xT=100,yT=100,zT=100) {
  data.frame(name = rep(mymodel$states, 3),
                   time = rep(c(0.001,2000,3000), each = length(mymodel$states)),
                   value = c(c(100,0,100,0,0,100,0),  #initial values
                             rep(newsteadystate(x2,y2,cyz,z2, xT,yT,zT),2) #final values
                   ),
...
pars_list <- list()
obs_fun_list <- list()
setting_description <- c("Small complex concentrations",
                         "Medium small complex concentrations",
                         "Medium complex concentrations",
                         "Large complex concentrations")
```

## Setting 1: Small complex concentrations
```{r Setting 1}
obs_fun_list[[1]]   <-  g0
pars_perturbed_list[[1]] <- c(x1 = 0.5, y1 = 0.5, z1 = 0.5)

data <- data_fun(40,50,3,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[1]] <- pars <- (myfit %>% 
                             as.parframe() %>% 
                             as.parvec() %>% 
                             p_nosyndeg() %>% 
                             extract2(1))

ic_list_list[[1]] <- ic_list <- list(gainlf = c(1,0,5),
                                     gainuf = c(1,0,5),
                                     gainyf = c(1,0,5))
...
## Setting 2: Medium complex concentrations
```{r Setting 2}
obs_fun_list[[2]]   <-  g0
pars_perturbed_list[[2]] <- c(x1 = 0.5, y1 = 0.5, z1 = 0.5)

data <- data_fun(50,50,20,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[2]] <- pars <- (myfit %>% 
                             as.parframe() %>% 
                             as.parvec() %>% 
                             p_nosyndeg() %>% 
                             extract2(1))

ic_list_list[[2]] <- ic_list <- list(gainlf = c(1,0,5),
                                     gainuf = c(1,0,5),
                                     gainyf = c(1,0,5))
...
## Setting 3: Large complex concentrations
```{r Setting 3}
obs_fun_list[[3]]   <-  g0
pars_perturbed_list[[3]] <- c(x1 = 0.5, y1 = 0.5, z1 = 0.5)

data <- data_fun(50,40,50,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[3]] <- pars <- (myfit %>% 
                             as.parframe() %>% 
                             as.parvec() %>% 
                             p_nosyndeg() %>% 
                             extract2(1))

ic_list_list[[3]] <- ic_list <- list(gainlf = c(1,0,5),
                                     gainuf = c(1,0,5),
                                     gainyf = c(1,0,5))
...
## Setting 4: Small complex concentrations, overexpression
```{r Setting 4}
obs_fun_list[[4]]   <-  g0
pars_perturbed_list[[4]] <- c(x1 = 1.5, y1 = 1.5, z1 = 1.5)

data <- data_fun(50,50,3,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[4]] <- pars <- (myfit %>% 
                             as.parframe() %>% 
                             as.parvec() %>% 
                             p_nosyndeg() %>% 
                             extract2(1))

ic_list_list[[4]] <- ic_list <- list(gainlf = c(1,0,5),
                                     gainuf = c(1,0,5),
                                     gainyf = c(1,0,5))
...
## Setting 5: Small complex concentrations, overexpression
```{r Setting 5}
obs_fun_list[[5]]   <-  g0
pars_perturbed_list[[5]] <- c(x1 = 0.5, y1 = 0.5, z1 = 0.5)

data <- data_fun(50,50,20,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[5]] <- pars <- (myfit %>% 
                             as.parframe() %>% 
                             as.parvec() %>% 
                             p_nosyndeg() %>% 
                             extract2(1))

ic_list_list[[5]] <- ic_list <- list(gainlf = c(1,0,5),
                                     gainuf = c(1,0,5),
                                     gainyf = c(1,0,5))
...
## Setting 6: Small complex concentrations, overexpression
```{r Setting 6}
obs_fun_list[[6]]   <-  g0
pars_perturbed_list[[6]] <- c(x1 = 0.5, y1 = 0.5, z1 = 0.5)

data <- data_fun(50,40,50,30)
obj <- normL2(data,(x*p_nosyndeg))
myfit <- mstrust(obj, pouter_0, cores = 3, fits = 3, studyname = "FindPars", resultPath = ".Findpars")
pars_list[[6]] <- pars <- (myfit %>% 
                             as.parframe() %>% 
                             as.parvec() %>% 
                             p_nosyndeg() %>% 
                             extract2(1))

ic_list_list[[6]] <- ic_list <- list(gainlf = c(1,0,5),
                                     gainuf = c(1,0,5),
                                     gainyf = c(1,0,5))
...
```{r Analyze Fits to knitr-Document noisy, results='asis', out.width='90%', warning=FALSE, message=FALSE}
library(tidyr)
library(dplyr)
wup <- lapply(seq_along(pars_list), function(setting) {
  ...
  output <- lapply(multiple_fits[[setting]], function(cond) {
    ...
    cat("\\newpage
        ...
        \\section{Condition: ",(cond$condition %>% str_split("_") %>% extract2(1) %>% paste(collapse = ", ") %>% str_replace_all("=", " = ")), "} \n" )
    cat("\\textbf{Perturbed Parameters}: ", cond$pars_perturbed, "\n" )
