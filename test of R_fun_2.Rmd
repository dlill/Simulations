...
r_names_0 <- paste0("r",outer(1:2,1:2, paste0))
...
# Test the same for the modularization when the complex is added to the ERK-Module
```{r}
perturbation_prediction <-  (xs*p_pert)(times = c(0,Inf), pars = pars, deriv = F)
alpha_scan_range <- seq(-2,4,by = 0.1)
r_names_0 <- paste0("r",outer(1:2,1:2, paste0))

r_df <- sapply(alpha_scan_range, function(par_opt) {
  pars_opt <- structure(10^(par_opt), names = c("a_1"))
  myr <- R_fun(pars_opt = pars_opt, p_fun = (p_pert), obs_fun = g2) %>% local_response_matrix_eq10()
})%>%
  t %>%
  structure(. , dimnames = list(NULL, r_names_0)) %>%
  cbind(log10alpha = alpha_scan_range, .) %>%
...
r_2_df <- sapply(alpha_scan_range, function(par_opt) {
  pars_opt <- structure(10^(par_opt), names = c("a_1"))
  myr <- R_fun_2(pars_opt = pars_opt, obs_fun = g2) %>% local_response_matrix_eq10()
})%>%
  t %>%
  structure(. , dimnames = list(NULL, r_names_0)) %>%
  cbind(log10alpha = alpha_scan_range, .) %>%
