...
mydoseresponse <- lapply(seq(-1,1, by = .1), function(i) {
  pars <- pars_0
  pars["a_1"] <- 1
...
mydoseresponse %>% 
  mutate(log_ratio_complex_m2 = log(Cm2e1/m2)) %>% 
  gather(key = key,value =value , deriv, r11,r21,r12,r22) %>% 
  ggplot(aes(x = log_ratio_complex_m2, y = value)) +
  geom_line() +
...
mydoseresponse %>% 
  ...
  mutate(x = m2P, y = e2P) %>% 
  gather(key = key,value =value , deriv, r11,r21,r12,r22, x,y,m2,Cm2e1, log_ratio_complex_m2) %>% 
...
mydoseresponse %>% 
  ...
  # mutate(deriv = log10(deriv)) %>%
  # gather(key = key,value =value , deriv, r11,r21,r12,r22) %>% 
  ggplot(aes(x = log10(r12), y = deriv)) +
  geom_path() 
  # facet_wrap(~key, scales = "free")
```
