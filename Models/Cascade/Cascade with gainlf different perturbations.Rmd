...
}, machine = c("knecht2"), input = ls(pos=.GlobalEnv), filename = "2018_07_18_33_10_gainlf"
, recover = T
)
# myrunbg_job$check()
...
}, machine = c("knecht2"), input = ls(pos=.GlobalEnv), filename = "2018_07_18_33_10_gainlf2"
, recover = T
)
# myrunbg_job$check()
# myrunbg <- myrunbg_job$get()[[1]]
# saveRDS(myrunbg, file = "2018_07_18_33_10_gainlfmyrunbg2.rds")
results2 <- readRDS("2018_07_18_33_10_gainlfmyrunbg2.rds")
results <- readRDS("2018_07_18_33_10_gainlfmyrunbg.rds")
...
myrunbg_job <- runbg({
  ...
  gainlfs <- seq(-0.5,0.5,.1) %>% unique() %>% sort
  map(gainlfs, function(gainlf) {
    pars <- pars0
    pars["gainlf"] <- c(gainlf = gainlf)
...
# myrunbg_job$check()
# myrunbg <- myrunbg_job$get()[[1]]
# saveRDS(myrunbg, file = "2018_07_18_33_10_gainlfmyrunbg3.rds")
results3 <- readRDS("2018_07_18_33_10_gainlfmyrunbg3.rds")
results2 <- readRDS("2018_07_18_33_10_gainlfmyrunbg2.rds")
results <- readRDS("2018_07_18_33_10_gainlfmyrunbg.rds")
...
cluster_matrices_and_print_perturbations <- function(results_unnested_matrices, a, glf, col_start, nclust, which_clust) {
  kclusts <- results_unnested_matrices %>% 
    filter(near(alpha, a), near(gainlf, glf)) %>% 
  ...
  augmented <- broom::augment(kclusts, results_unnested_matrices %>% filter(near(alpha, a), near(gainlf, glf)))
...
results_unnested_matrices %>% select(alpha, gainlf) %>% map(unique)
```
...
results %>% 
  filter(alpha == 0, gainlf %in% c(-10,0,2), map_lgl(pars_perturbed,. %>% {str_detect(names(.), c("x1", "y1", "z1"))} %>% all)) %>% 
  .[c("r_opt","r_opt1")] %>%
  unlist(F) %>% #.[t(matrix(1:6, nrow = 3))] %>% 
  {wup <- .; `names<-`(wup, names(wup) %>% str_replace_all(c("1$" = "gl = -10", "2$" = "gl = 0", "3$" = "gl = 2",
                                               "r_0" = "a = 0, ", "r_1" = "a = 1, ", "r_opt" = "a = a_opt, ")))} %>%
  imap(~ .x  %>% round(2) %>% as.tibble() %>% `names<-`(mynames) %>% bind_cols(aopt = rep(.y ,3), module = mynames,.) ) %>% mypublish4("matrices_gainlf_positive_detected")


mynames <- paste0(LETTERS[24:26],"*")
results %>% 
  filter(alpha == 0, gainlf %in% c(-10,0,2), map_lgl(pars_perturbed,. %>% {str_detect(names(.), c("x1", "y1", "k8"))} %>% all)) %>% 
  .[c("r_opt","r_opt1")] %>%
  unlist(F) %>% #.[t(matrix(1:6, nrow = 3))] %>% 
  {wup <- .; `names<-`(wup, names(wup) %>% str_replace_all(c("1$" = "gl = -10", "2$" = "gl = 0", "3$" = "gl = 2",
                                               "r_0" = "a = 0, ", "r_1" = "a = 1, ", "r_opt" = "a = a_opt, ")))} %>%
  imap(~ .x  %>% round(2) %>% as.tibble() %>% `names<-`(mynames) %>% bind_cols(aopt = rep(.y ,3), module = mynames,.) ) %>% mypublish4("matrices_gainlf_negative_detected")
...
* gain -10: There are basically two clusters, one where (r23 == 0) and one where (r23 != 0). for 5 clusters, the third one is the zero-cluster
...
```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 0, -10, "up_", 5, 5)%>% 
  print(n = 1000)
```
...
* r23 != 0: the rest
```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 1, 1, "up_", 6, 2)%>% 
  # select(Y) %>% unique() %>% t
  print(n = 1000)
```
...
```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 1, 2, "up_", 6, 3:6)%>% 
  print(n = 1000)
