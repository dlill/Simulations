...
dose_pars <- log(c(0.001, 0.5, 2, 10))
names(dose_pars) <- rep("u1", length(dose_pars))
alphas <- c(0, 1)
...
remote_computing_job <- runbg({
  ...
  results 
}, machine = ssh_machine, input = ls(pos=.GlobalEnv), filename = "cascade_zx_feedback"
# , recover = T
)
```
...
saveRDS(results, file = "results_cascade_zx_feedback.rds")
results <- readRDS("results_cascade_zx_feedback.rds")
```
...
results_unnested_matrices %>% 
  filter(dose_par == log(0.5)) %>% 
cluster_matrices_and_print_perturbations(0, "upstream_ropt", 2,NULL)
```
...
reactions %>% 
  as.eqnvec() %>% 
  print2word_eqnvec("odes_cascade_zx_fb")
```

```{r}
pars %>% 
  exp %>% 
  print2word_namedvec("pars_cascade_zx_fb", names_in_mathenv = T)
```

## Export matrices for the main text
```{r}
mypars <- pars
mypars["u1"] <- log(0.5)
matrices_weak_fb <- algo(mypars, paste0(letters[24:26], "1"), c("a_tox_Cxy", "a_toy_Cyz"), alpha_pars, 0, g, xs, p_log) %>% unlist(F) %>% .[-4]

print2word_matrixlist(matrices_weak_fb, "cascade_zx_feedback")
```

```{r}
mypars <- pars
mypars["u1"] <- log(0.5)
matrices_weak_fb <- algo(mypars, paste0(letters[24:26], "1"), c("a_Cxy", "a_Cyz"), alpha_pars, 0, g, xs, p_log) %>% unlist(F) %>% .[-4]

print2word_matrixlist2(matrices_weak_fb, "cascade_zx_feedback_CS20")
```
