...
source("/home/denial/Promotion/Projects/MRA/Simulations/S2 different settings-noisy.R")
```
...
remote_computing_job <- runbg({
  ...
  results <- run_different_perturbations_noisy(dose_pars = dose_pars, 
                                         pars0 = pars, 
...
print_distribution_plots <- function(results, a, s, perturbation_strength) {
  plotopt <- results %>% 
    ...
    geom_blank() 
  ggsave(paste0("r_opt_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, ".png"), plotopt)
  ggsave(paste0("r_opt_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, "coord2", ".png"), plotopt +
           coord_cartesian(ylim = c(-2,2)))
  ...
  plot0 <- results %>% 
    ...
    geom_blank() 
  ggsave(paste0("r_0_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, ".png"), plot0)
  ggsave(paste0("r_0_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, "coord2", ".png"), plot0 +
           coord_cartesian(ylim = c(-2,2)))
  ...
  plot1 <- results %>% 
    ...
    geom_blank() 
  ggsave(paste0("r_1_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, ".png"), plot1)
  ggsave(paste0("r_1_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, "coord2", ".png"), plot1 +
           coord_cartesian(ylim = c(-2,2)))
  ...
  plotkept <- results %>% 
    ...
    geom_blank() 
  ggsave(paste0("r_kept_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, ".png"), plotkept)
  ggsave(paste0("r_kept_srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", alpha, "coord2", ".png"), plotkept +
           coord_cartesian(ylim = c(-2,2)))
...
When the complex is measured independently, some combinations of parameter perturbations don't work out anymore, but most of them do.
```{r}
