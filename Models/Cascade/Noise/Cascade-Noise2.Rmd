...
asp <- expand.grid(a = alphas[2:4], s = map_dbl(error_pars, 2), perturbation_strength = map_dbl(perturbations,1))
...
walk(seq_len(nrow(asp)), function(.x) {
  pdf(paste0("srel_",s, "delta_p_", round(exp(perturbation_strength),2), "_alpha_", a, ".pdf"))
...
results  %>% 
  ...
  filter(!(map_lgl(r_0, is.null) ||
             ...
             map_lgl(r_optboth, is.null))) %>% nrow
# .$r_0 %>% map(dim) %>% reduce(rbind)
```
...
results_unnested_matrices <- results_unnested_matrices %>% 
  select(-r_1,-r_kept,-r_opt, -parframesupstream, -parframesboth,-parframes) %>% 
