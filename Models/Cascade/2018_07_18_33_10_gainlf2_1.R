...
try(library(methods))
try(library(base))
setwd('~/2018_07_18_33_10_gainlf2_1_folder')
rm(list = ls())

load('2018_07_18_33_10_gainlf2.RData')
...
.runbgOutput <- try({
    ...
    pars0 <- pars
    gainlfs <- seq(-0.5, 0.5, 0.1) %>% unique() %>% sort
        ...
        map(alphas, function(alpha) {
            mclapply(pp_list, function(pp) {
                ...
                algo <- function(which_pars_opt, prefix = NULL) {
                  ...
                  myfits <- mstrust(obj_alpha, center = structure(rep(0, length(which_pars_opt)), names = which_pars_opt), studyname = "Fits", cores = 1, fits = 6, sd = 1, mypars = pars, perturbation_prediction = perturbation_prediction, r_kept = r_kept, p_fun = (p * p_pert), obs_fun = g)
...
save(.runbgOutput, file = '2018_07_18_33_10_gainlf2_1_result.RData')