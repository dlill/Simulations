...
pars_perturbed <- c("x1" = log(0.9), "y1" = log(0.9), "z1" = log(0.9))
...
par_opt_names <- c("a_tox_Cxy", "a_toy_Cyz", "a_toz_Czx")


r_0 <-   R_fun(pars_opt = pars_opt[par_opt_names],
               perturbation_prediction = perturbation_prediction,
               obs_fun = g,
               p_fun = (p_log*p_pert),
               pars = pars) %>% local_response_matrix_eq10()
...
r_1 <-   R_fun(pars_opt = pars_opt[par_opt_names]-(log(0+.Machine$double.eps)),
               perturbation_prediction = perturbation_prediction,
               obs_fun = g,
               p_fun = (p_log*p_pert),
               pars = pars) %>% local_response_matrix_eq10()
...
myfits <- mstrust(obj_alpha, center =  par_opt_names %>% are_names_of(0), studyname = "Fits", cores = 3, fits = 3, sd = 1, mypars = pars, perturbation_prediction = perturbation_prediction, r_kept = r_kept_0)
# myfits %>% as.parframe() 
best_fit <- myfits %>% as.parframe() %>% as.parvec()

r_opt_1 <- r_alpha_fun(pars_opt = unclass(best_fit), pars = pars, perturbation_prediction = perturbation_prediction) %>% round(2)
...
r_0 %>% round(2) %>% 
  cbind(rep("", nrow(.)), paste0(rownames(.), "*"),.) %>% 
  `rownames<-`(NULL) %>% 
...
mypublish_mat2 <- function(x,filename) {
...
write2latexandcompile <- function(string, filename) {
  string %>% 
paste("\\documentclass[10pt,a4paper]{article}
\\usepackage[latin1]{inputenc}
\\usepackage{amsmath}
...
list(r_0 = r_0, r_1 = r_1, r_opt_1 = r_opt_1) %>% map(prepare_matrix_for_latex) %>% paste(collapse = "\n") %>% write2latexandcompile("fig4c")
