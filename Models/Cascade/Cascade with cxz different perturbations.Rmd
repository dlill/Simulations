...
cluster_matrices_and_print_perturbations <- function(results_unnested_matrices, a, col_start, nclust, not_which_clust) {
  ...
  augmented %>% 
    filter(! (.cluster %in% not_which_clust)) %>% 
    select(X,Y,Z) 
}
...
```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 0, "up_", 10, 1:2)%>% 
    print(n = 1000)
```

## Both

* Adding the complexes to both introduces the need to choose a different alpha, e.g. with alpha = 0, it doesn't work anymore:
```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 0, "both_", 20, 1:2)%>% 
    print(n = 1000)
```

* with higher alpha, it starts to work a bit better
```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 3, "both_", 15, (1:15)[-3])%>% 
    print(n = 1000)
cluster_matrices_and_print_perturbations(results_unnested_matrices, 3, "both_", 15, 3)%>% 
    print(n = 1000)
```

```{r}
cluster_matrices_and_print_perturbations(results_unnested_matrices, 3, "both_", 20, (1:20)[-3]) %>% 
  arrange(X,Y,Z) %>% 
  print(n = 1000)
```
