...
```{r}
length(alphas) * length(perturbations) * length(alpha_par_settings) * length(error_pars) * N
```
...
remote_computing_job <- runbg({
  ...
  results 
}, machine = ssh_machine, input = ls(pos=.GlobalEnv), filename = "cascade3rep_rechecked"
, recover = T
)
```
...
results_unnested_matrices <- results %>%
  ...
  unnest(r_optupstream,# r_optboth,
...
results_unnested_matrices <- results_unnested_matrices %>% 
  select(-parframesupstream) %>%
  gather(key, value, matches("upstream|r_0")) %>% 
  mutate(par_opt_setting = str_extract(key, "upstream"),
...
```{r}
dir.create("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicatesRechecked/plots2")
setwd("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicatesRechecked/plots2")
...
# Confirm that the right elements are kept in the optimization
```{r}
s <- 0.01
perturbation_strength <- log(0.5)

print_r_kept_plots <- function(results_unnested_matrices, s, perturbation_strength) {
  p1 <- results_unnested_matrices %>% 
    filter(srel == s) %>%
    filter(map_lgl(pars_perturbed, ~near(.x[1], perturbation_strength))) %>%
    filter(par_opt_setting == "upstream"|is.na(par_opt_setting)) %>%
    # filter(!mat %in% c("r_kept","r(a=1)")) %>%
    ...
    filter(mat == "r_kept") %>% 
    mutate(mat = factor(mat),
           value = factor(value, levels = c(0,1), labels = c("not minimized", "minimized")))%>% 
    ggplot(aes(x = value)) +
    # geom_violin(draw_quantiles = 0.5) +
    # geom_boxplot() +
    geom_bar()+
  ...
  ggsave(paste0("count_n_minimized-", s, "_deltap-", round(exp(perturbation_strength),2), ".png"), p1)
}

setwd("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicates/plots2")
sp <- expand.grid(s = c(0.001,0.01,0.05,0.1), perturbation_strength = log(c(0.9,0.5)))
walk(seq_len(nrow(sp)), function(.x) {
  with(sp[.x,], print_r_kept_plots(results_unnested_matrices, s, perturbation_strength))
})
