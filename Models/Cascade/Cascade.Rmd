...
Since all possible combinations of paramter perturbations are many, these computations are done in a parallelized manner on a remote server. This is done by "rungb()". 
```{r}
...
# saveRDS(myrunbg, file = "results_cascade_different_perturbations.rds")
results <- readRDS("results_cascade_different_perturbations.rds")
...
```{r}
cluster_by_r_0 <- cluster_matrices_and_print_perturbations(results_unnested_matrices, 0, "r_0", 8,NULL)
```
Also 4 clusters when total phosphorylated species are measured.
```{r}
cluster_by_r_1 <- cluster_matrices_and_print_perturbations(results_unnested_matrices, 0, "upstream_r1", 8,NULL)
```
