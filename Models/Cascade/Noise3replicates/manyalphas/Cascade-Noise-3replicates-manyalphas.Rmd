...
remote_computing_job <- runbg({
  ...
  results 
}, machine = ssh_machine, input = ls(pos=.GlobalEnv), filename = "cascade3rep_manyalphas"
, recover = T
)
```
...
results_unnested_matrices %>% 
  ...
  filter(abs(value)<10) %>% 
  ggplot(aes(x = alpha, y = value, color = factor(realization)), alpha = 0.5) + 
    geom_line() + 
...
print_distribution_plots <- function(results_unnested_matrices, s, perturbation_strength) {
  ...
  ggsave(paste0("ralpha_vs_alpha-", s, "_deltap-", round(exp(perturbation_strength),2), ".png"), p1)
  
}

dir.create("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicates/manyalphas/plots_ralphavsalpha")
setwd("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicates/manyalphas/plots_ralphavsalpha")
...
```{r}
dir.create("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicates/manyalphas/plots")
setwd("~/Promotion/Projects/MRA/Simulations/Models/Cascade/Noise3replicates/manyalphas/plots")
sp <- expand.grid(s = c(0.001,0.01,0.05,0.1), perturbation_strength = log(c(0.9,0.5)))
walk(seq_len(nrow(sp)), function(.x) {
  with(sp[.x,], print_distribution_plots(results_unnested_matrices, s, perturbation_strength))
})
