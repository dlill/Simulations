...
latexTables <- function(test, purplifyIdxs = NULL) {
  wup <- unlist(test, recursive = F)[c(1,2,3,5)]
  if (length(purplifyIdxs)) wup <- lapply(wup, function(x) {x[purplifyIdxs] <- paste0("\\mathbf{\\color{darkRed}  ", x[purplifyIdxs], "}");x})
  wup <- lapply(wup, function(x) xtable::xtable(x))
  wup <- lapply(wup, function(x) capture.output(print(x,
  ...
  wup <- lapply(wup, function(x) {x <- x[-(1:2)]; x[1] <- "$\\begin{pmatrix}"; x[5] <- paste0(x[5], "$");x})
  lapply(wup[c(1,2,4)], function(x) cat(c(x, "&"), sep = "\n"))
}
...
postProcess(test)
latexTables(test, purplifyIdxs = c(3,4,8))
```
