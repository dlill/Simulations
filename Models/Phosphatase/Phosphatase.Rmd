...
```{r, warning=FALSE}
par_opt_settings <- list("a_xy", "a_tox_xy", "a_toy_xy", c("a_tox_xy", "a_toy_xy")) %>% `names<-`(1:4)
...
## Plot adding the complex to lower module
```{r}
plot_data <-
  dr_list %>%   
  combine_dr_list() %>% 
...
myplot
ggsave("plot2lower.png", myplot)
print_matrices_at_point_locations(point_locations, plot_data, path = "matrices_of_plot2lower_Phosphatase")
```
