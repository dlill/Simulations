...
list(r_0 = r_0, r_1 = r_1, r_opt = r_opt) %>% 
  imap(~ .x  %>% round(2) %>% as.tibble() %>% `names<-`(mynames) %>% bind_cols(aopt = rep(.y ,3), module = mynames,.) ) %>% mypublish4("matrices_v1k31z1")

list(r_0 = r_0, r_1 = r_1, r_opt = r_opt) %>% 
  imap(~ .x  %>% round(2) %>% xtable(align = rep("",4)) %>% print(tabular.environment="pmatrix", include.colnames = F, include.rownames = F, floating = F, hline.after = NULL) %>%  mypublish_mat(paste0("matrix", .y, "v1k31z1")))
