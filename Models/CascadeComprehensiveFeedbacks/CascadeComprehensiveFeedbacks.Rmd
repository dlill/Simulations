...
## Parameter set 3, compare different ways of global responses
...
# generate functions for the different experimental conditions
pars_perturbed_0 <- pars_perturbed <- c("x1" = log(1.001), "y1" = log(1.001), "z1" = log(1.001))
p_pert <- p_pert_fun(pars_perturbed = pars_perturbed, pars = pars)
...
cat("out\n")
(xs*p*p_pert)(c(0,Inf), pars, deriv = F) %>% do.call(rbind,.) %>% round(1)
...
# extract_derivs((g* x*p*p_pert)(times, pars, conditions = "Ctr"), modules0, paste0(letters[24:26], "1")) %>% 
# as.prdlist() %>%  plotCombined()
...
diag_xi <- (g*xs*p*p_pert)(c(0,Inf), pars, conditions = "Ctr", deriv = F) %>% .[[1]] %>% .[-1] #%>% diag()

# (x*p*p_pert)(seq(0,20000,length.out = 100), pars, conditions = "Ctr") %>% 
  # .[[1]]


cat("sensitivities\n")
r_sens <- 
  conveniencefunctions::extract_derivs((g* xs*p*p_pert)(c(0,Inf), pars, conditions = "Ctr"), modules0, paste0(letters[24:26], "1")) %>%
  # conveniencefunctions::extract_derivs((g*x*p*p_pert)(seq(0,20000,length.out = 100), pars, conditions = "Ctr"), modules0, paste0(letters[24:26], "1")) %>%
    # conveniencefunctions::extract_derivs((x*p*p_pert)(seq(0,20000,length.out = 100), pars, conditions = "Ctr"), NULL, paste0(letters[24:26], "1")) %>%
  ...
  round(2) %>% 
  # sign %>% 
  magrittr::set_rownames(modules0) %>%
...
cat("\n\n 1.001 fold perturbation\n\n")

# R_fd: Finite differences
R_fd <- function(pars_opt = pars_opt_0,
  ...
  sapply(2:length(my_perturbation_prediction), function(i) {
    dp <- (mypars[[i]]-mypars[[1]]) %>% {.[abs(.)>0]} %>% {attributes(.) <- NULL;.}
    Ctr <- my_perturbation_prediction[[1]][,-1]
    perturbed <- my_perturbation_prediction[[i]][,-1]
    return((log(perturbed)-log(Ctr))/dp)
  })
}

cat("R_fd\n")
bla <- R_fd(pars_opt = pars_opt,
            perturbation_prediction = perturbation_prediction,
            obs_fun = g,
            p_fun = (p*p_pert),
            pars = pars) %>% 
...
# R_2: Just the log-foldchange: log(y(x+dx)/y(x))
R_2 <- function(pars_opt = pars_opt_0,
                  perturbation_prediction = perturbation_prediction,
                  obs_fun = g,
                  p_fun = (p*p_pert),
                  pars = pars_0) {
  ...
  sapply(2:length(my_perturbation_prediction), function(i) {
    ...
    return((log(perturbed)-log(Ctr))[,-1])
  })

}

cat("R2\n")
bla <- R_2(pars_opt = pars_opt,
            perturbation_prediction = perturbation_prediction,
            obs_fun = g,
            p_fun = (p*p_pert),
            pars = pars) %>% 
...
cat("\n\n1.1 fold perturbation\n\n")

# generate functions for the different experimental conditions
pars_perturbed_0 <- pars_perturbed <- c("x1" = log(1.1), "y1" = log(1.1), "z1" = log(1.1))
p_pert <- p_pert_fun(pars_perturbed = pars_perturbed, pars = pars)
...
# generate functions for the different experimental conditions
pars_perturbed_0 <- pars_perturbed <- c("x1" = log(2), "y1" = log(2), "z1" = log(2))
p_pert <- p_pert_fun(pars_perturbed = pars_perturbed, pars = pars)
...
## what the fuck, sensitivities?
```{r}
steady_state <- perturbation_prediction[[1]][,-1]
steady_state <- steady_state[steady_state!=0]
states <- names(steady_state) %>% setNames(.,.)
p <- trafo %>% 
  define("x~x", x= states) %>% insert("x~exp(x)", x= states) %>% 
  define("x~0", x = str_subset(getSymbols(mytrafo[[i]]), "^a")) %>% 
  define("x~0", x = c("Cz2y2","Cy2x2","Cz2x2","Cz2x1")) %>% 
  P

(g*x*p)(seq(0,20000,length.out = 100), log(steady_state))
```


```{r}
getParameters(x) %>% length()
pinner %>% length()

inits <- compare(names(pinner), getParameters(x)) %>% .[11:19,] %>% unlist() %>% as.character() %>% are_names_of(0)

mypars <- c(pinner, inits)
mypars[names(states)] <- steady_state

glog <- Y(c(X = "log(x2)", "Y" = "log(y2)", "Z" = "log(z2)"),x)


# (glog*x)(seq(0,20000,length.out = 100), mypars) %>%
#   extract_derivs(which_pars = outer(letters[24:26], 1:2, paste0) %>% .[T]
#                  # , which_states = outer(letters[24:26], 1:2, paste0) %>% .[T]
#                  ) %>% 
# .[[1]] %>% .[100,]
r_sens_richtig <- (glog*x*p)(seq(0,20000,length.out = 100), log(steady_state)) %>%
  extract_derivs(which_pars = outer(letters[24:26], 1, paste0) %>% .[T]
                 # , which_states = outer(letters[24:26], 1:2, paste0) %>% .[T]
                 , which_states = modules0
                 ) %>% 
.[[1]] %>% .[100,-1] %>% 
  matrix(ncol = 3) %>%
