...
```{r which pars are perturbed, echo=FALSE}
...
r_kept_fun_1 <-function(pars_opt = pars_opt_0,
  ...
  return(cbind(keep, rep(0, nrow(keep)), r_alpha))
}

cat("Left matrix: Elements to keep, when comparing alpha = 0 and alpha = alpha \n
     Right matrix: r_alpha \n")
bla <- lapply(10^(-4:8), function(a) {
  cat(" alpha = ", a, "\n")
  print(xtable(r_kept_fun_1(obs_fun = g0, alpha = a)), comment = F, floating = F)
  cat(" ")
  }) 
cat("Only complexes")
print(xtable(r_kept_fun_1(obs_fun = g2, alpha = 0)), comment = F, floating = F)
  cat(" ")
