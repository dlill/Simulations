...
pars_opt_0 <- pars_opt <- structure(rep(-16, length(pars_inner_opt_0)), names = paste0("log", names(pars_inner_opt_0)))
...
pars_perturbed_0 <- pars_perturbed <- c(logm1 = 1.1, loge1 = 1.1)
...
```{r Fit, eval=FALSE}
logk1 <- 0
bla <- sapply(seq(-5,1, by = 1), function(logk1) {
  pars <- pars_0
  pars["logk1"] <- logk1 %>% set_names("logk1")
perturbation_prediction <-  (xs*p_log*p_pert)(times = c(0,Inf), pars = pars, deriv = F)



myfits <- mstrust(obj_alpha,
                  center =  pars_opt_0[1]-pars_opt_0[1], 
                  studyname = "Fits", 
                  cores = 3, 
                  fits = 3, 
                  sd = 3, 
...
c(myfits %>% as.parframe() %>% as.parvec(), log(exp(pars["logk1"])/(exp(pars["logk2"])+exp(pars["logk1"]))))
})
bla
# pars["k1"] <- 
...
```{r}
par_opt_settings <- list(1,2,c(1,2)) %>% set_names(1:3)
dr_list <- lapply(par_opt_settings, function(i) {
  r_opt_dose_response(which_par = "logk1", 
                      dosages = pars_0["logk1"],
                      pars_opt = pars_opt_0[i],
                      alpha_scan_range = sort(unique(seq(-3,4,by = 1)))
  )
})
...
myprof <- profile(obj_alpha, best_fit, names(best_fit), limits = c(-15,15))
plotProfile(myprof)
